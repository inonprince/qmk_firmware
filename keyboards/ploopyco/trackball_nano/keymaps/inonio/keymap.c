...
// however, macos ingonre fast caps lock tapping, so we set our macros to longer presses 
#define LED_CMD_TIMEOUT 400
...
static int8_t intentional_movement_counter = 0;

static deferred_token scroll_lock_timer;
static deferred_token intentional_movement_timer;
static deferred_token cmd_timeout_timer;
...
uint32_t scroll_lock_timeout(uint32_t trigger_time, void *cb_arg) {
    // if (host_keyboard_led_state().scroll_lock) {
        ...
        uprint("tapping scroll timeout)\n");
    // }
    scroll_lock_timer_enabled = false;
    return 0; // Don't repeat
}

uint32_t intentional_movement_timeout(uint32_t trigger_time, void *cb_arg) {
    uprint("intentional_movement_timeout)\n");
    intentional_movement_counter = 0;
    return 0; // Don't repeat
}

report_mouse_t pointing_device_task_user(report_mouse_t mouse_report) {
    // if (mouse_report.x != 0 || mouse_report.y != 0) uprintf("%d %d\n", mouse_report.x, mouse_report.y);
    if (mouse_report.x != 0 || mouse_report.y != 0) {
        if (scroll_lock_timer_enabled) {
            // uprint("extend_deferred_exec)\n");
            extend_deferred_exec(scroll_lock_timer, SCROLL_LOCK_TIMEOUT);
        } else {
            if (intentional_movement_counter > 1) {
                cancel_deferred_exec(intentional_movement_timer);
                intentional_movement_counter = 0;
                // if (!host_keyboard_led_state().scroll_lock) {
                tap_code(KC_SCROLL_LOCK);
                uprint("tapping scroll)\n");
                // }
                scroll_lock_timer_enabled = true;
                scroll_lock_timer = defer_exec(SCROLL_LOCK_TIMEOUT, scroll_lock_timeout, NULL);
            } else if (intentional_movement_counter > 0) {
                intentional_movement_counter++;       
            } else {
                intentional_movement_counter++;
                intentional_movement_timer = defer_exec(100, intentional_movement_timeout, NULL);
            }
        }
    }
    if (scroll_enabled) {
...
bool led_update_user(led_t led_state) {
    ...
    if (!in_cmd_window) {
        in_cmd_window = true;
        cmd_timeout_timer = defer_exec(LED_CMD_TIMEOUT, command_timeout, &cmd_window_state);
    }
